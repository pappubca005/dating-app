from django.shortcuts import render, reverse, redirect, render
...
def new_message_check(request):
    ...
    is_read = Messages.objects.filter(
        conversation=conversation_id, receiver=request.user, is_read=False
    ).exists()
    data = {"conversation": is_read}
...
def chat(request, id):
    ...
    for conversation in conversation_ids:
        all_conversations[conversation.id] = Messages.objects.filter(
            conversation=conversation
        ).last()

        # Is the last message recieved read? Used to update styling.
        last_message = Messages.objects.filter(
            conversation=conversation, receiver=request.user
        ).last()
        if last_message:
...
def wink(request):
    ...
    current_wink = Winks.objects.filter(
        Q(receiver_id=receiver_id) & Q(sender_id=request.user.id) & Q(is_read=False)
    ).exists()
    if current_wink:
        data = {}
...
def reject(request):
    ...
    current_reject = Winks.objects.filter(
        Q(receiver_id=receiver_id) & Q(sender_id=request.user.id)
    ).exists()
    if current_reject:
        return HttpResponse(status=204)
...
def chat_ajax(request):
    ...
    conversation = Conversations.objects.filter(participants=request.user.id).filter(
...
def winks(request):
    ...
    context = {"page_ref": "wink", "winks_page": winks_page, "page": page}
...
def views(request):
    ...
    context = {"page_ref": "view", "views_page": views_page, "page": page}
